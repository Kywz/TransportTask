...
namespace TransportTask.Transport.TransportTable
{
    public class DataProcessor
    {
        ...
        public static void northWestAngleMethodSollution(List<DataGridView> dataInput, Panel solutionPanel, List<DataGridView> dataOutput)
        {
            ...
            for (int i = 0; i < table.Y.Length; i++)
                for (int j = 0; j < table.X.Length; j++)
                {
                    ...
                    dataOutput.ElementAt(0).Rows[i].Cells[j].Value = (table.center[i, j] + "[" + Optimum[i, j] + "]").ToString();
                    fun += Optimum[i, j] * table.center[i, j];
                }
